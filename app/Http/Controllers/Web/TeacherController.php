	...
	public function dateils(){
		...
		if($this->data['is_nature'] == 1){
			...
			$data =Coures::leftJoin('ld_course_teacher','ld_course_teacher.course_id','=','ld_course.id')
						...
						->select('ld_course.cover','ld_course.title','ld_course.pricing','ld_course.buy_num','ld_lecturer_educationa.id','ld_course.id as course_id','ld_course_school.sale_price')
						->get()->toArray();
			if(!empty($data)){
