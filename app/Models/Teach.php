	...
	public static function getList($body){
		...
		$courseArr = CourseShiftNo::rightJoin('ld_course_class_number','ld_course_class_number.id','=','ld_course_shift_no.id')
					...
					->where(function($query) use ($body,$school_id) {
						...
						$query->where('ld_lecturer_educationa.type',2);
				})->select('ld_course_shift_no.name as classno_name','ld_lecturer_educationa.real_name as class_name','ld_course_class_number.start_at','ld_course_class_number.end_at','ld_lecturer_educationa.real_name as teacher_name','ld_course_live_childs.watch_num','ld_course_class_number.id as class_id','ld_course_class_number.shift_no_id as classon_id')
				->get()->toArray();
				$newcourseArr = [];
