...
class CourseStocks extends Model {
    ...
    public static function getCourseStocksList($data){
    	...
    	$residue_number = Order::whereIn('pay_status',[3,4])->where(['class_id'=>$data['course_id'],'school_id'=>$data['school_id'],'oa_status'=>1,'nature'=>1])->count();
        if(!empty($info)){
            foreach($info as $k=>$v){
     ...
     */
   	public static function doInsertStocks($data){
        ...
        // $CourseSchoolData = CourseSchool::where('id',$data['course_id'])->select('course_id')->first(); //前端传的course_id 为ld_course_school自增id
   		$sum_current_number = self::where('school_id',$data['school_id'])->where(['school_pid'=>$data['school_pid'],'is_del'=>0,'course_id'=>$data['course_id']])->orderBy('id','desc')->sum('add_number');//当前已经添加总库存

   		$residue_number = Order::whereIn('pay_status',[3,4])->where(['class_id'=>$data['course_id'],'school_id'=>$data['school_id'],'oa_status'=>1,'nature'=>1])->count(); //使用数量
