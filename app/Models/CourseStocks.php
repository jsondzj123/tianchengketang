...
class CourseStocks extends Model {
     ...
     */
   	public static function doInsertStocks($data){
        ...
        $CourseSchoolData = CourseSchool::where('id',$data['course_id'])->select('id')->first(); //前端传的course_id 为ld_course_school自增id
   		$sum_current_number = self::where('school_id',$data['school_id'])->where(['school_pid'=>$data['school_pid'],'is_del'=>0,'course_id'=>$CourseSchoolData['id']])->orderBy('id','desc')->sum('add_number');//当前已经添加总库存
        ...
        $data['course_id'] = $CourseSchoolData['id'];
